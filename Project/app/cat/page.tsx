...
export default function AllyCatsPage() {
  ...
  const filteredCats = allyCats.filter((cat) => {
    ...
    const matchesTarget =
      ...
      (targetFilterMode === 'OR'
        ? selectedTargets.some((t) => (cat.targetAttributes as string[]).includes(t))
        : selectedTargets.every((t) => (cat.targetAttributes as string[]).includes(t)));

    const matchesEffect =
      selectedEffects.includes('all') ||
      (effectFilterMode === 'OR'
        ? selectedEffects.some((e) => (cat.effects as string[]).includes(e))
        : selectedEffects.every((e) => (cat.effects as string[]).includes(e)));


    const matchesAbility =
      selectedAbilities.includes('all') ||
      (abilityFilterMode === 'OR'
        ? selectedAbilities.some((ab) => cat.abilities.includes(ab))
        : selectedAbilities.every((ab) => cat.abilities.includes(ab)));
  ...
  const getColorClasses = (color: string, isSelected: boolean) => {
    const colorMap: Record<string, { selected: string; hover: string }> = {
      ...
      black: { selected: 'bg-black text-white border-black', hover: 'hover:bg-zinc-100 hover:border-zinc-400' },
      emerald: { selected: 'bg-emerald-600 text-white border-emerald-600', hover: 'hover:bg-emerald-100 hover:border-emerald-400' },
      'blue-900': { selected: 'bg-blue-900 text-white border-blue-900', hover: 'hover:bg-blue-100 hover:border-blue-400' },
    };
